...
const GuideRequest = () => {
  ...
  const loadGuide = async () => {
    const res = await axios.get(`http://localhost:8070/guide/${id}`);
  ...
  const onSubmit = async (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      if (!/^\d+$/.test(noofdates)) {
        alert("Number of dates must be positive integers");
        return;
      }
      if (new Date(tourdate) < new Date()) {
        alert("Tour date should be greater than the current date.");
        return;
      }
    ...
    <div>
      ...
      <div className="info">
        <div className="request-container" style={{ background: "white" }}>
          <br />
          <div
            className="w-50 mx-auto shadow p-5 guide-container"
            ...
            >
              ...
              <center>
                <button type="submit" className="btn btn-dark">
                  <i
                    className="fa fa-share-square-o mr-2"
                    aria-hidden="true"
                  ></i>
                  Send Request
                </button>
